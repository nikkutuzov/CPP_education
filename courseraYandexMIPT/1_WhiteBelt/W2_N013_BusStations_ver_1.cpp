...
/*================================================================================*/
/*  Tittle: Bus stops ver. 1                                                      */
/*  Start_time: 24.06.2022 21:39                                                  */
/*  End_time: 25.06.2022 20:28                                                    */
/*  Note:                                                                         */
/*================================================================================*/
 ...
 *   команде NEW_BUS. Описание каждой остановки stop должно иметь вид
 *   Stop stop: bus1 bus2 ..., где bus1 bus2 ... - список автобусов, проезжающих
 *   через остановку stop, в порядке, в котором они создавались командами NEW_BUS,
 *   за исключением исходного маршрута bus. Если через остановку stop не проезжает
 *   ни один автобус, кроме bus, вместо списка автобусов для неё выведите
...
int main() {
  std::map<std::string, std::vector<std::string>> buses_for_stop;// ключ - остановка
  std::map<std::string, std::vector<std::string>> stops_for_bus;// ключ - автобус
  int count;
  std::cin >> count;
  for (int i = 0; i < count; ++i) {
    ...
    if (request == "NEW_BUS") {
      ...
      std::vector<std::string>& stops = stops_for_bus[bus];
      ...
      if (buses_for_stop.count(stop) == 0) {
        ...
        for (const std::string& item : buses_for_stop[stop]) {
          std::cout << item << " ";
        }
      }
      std::cout << std::endl;
      ...
      if (stops_for_bus.count(bus) == 0) {
        ...
        for (const std::string& item_stop : stops_for_bus[bus]) {
          std::cout << "Stop " << item_stop << ": ";
          if (buses_for_stop[item_stop].size() == 1) {
            std::cout << "No interchange";
          } else {
            for (const std::string& item_bus : buses_for_stop[item_stop]) {
              if (item_bus != bus) {
                std::cout << item_bus << " ";
              }
            }
